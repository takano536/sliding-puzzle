...
class App:
    ...
    def __init__(self):
        ...
        block_colors = color_gradator.get_colors(max([max(idxs) for idxs in config['map']]) + 1)
        block_colors.append(config['color']['space'])

        # init surface & dirty rects
        screen_width = len(config['map'][0]) * self.__tile_size[0] + self.__padding[0] * 2
        screen_height = len(config['map']) * self.__tile_size[1] + self.__padding[1] * 2
    ...
    def __mainloop(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_r:
                self.__field.reset(self.__surface, self.__dirty_rects)
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                self.__quit()
            if event.type == pygame.QUIT:
                self.__quit()

        self.__field.update(self.__surface, self.__dirty_rects, cursor, event_type, key)
