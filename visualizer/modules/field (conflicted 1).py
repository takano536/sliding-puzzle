...
class Field:
    ...
    def play_answer(self, surface: pygame.Surface, dirty_rects: list):
        ...
        self.__swap(block_idx=block_idx, key=key, surface=surface, dirty_rects=dirty_rects)
        self.__curr_answer_step += 1
    ...
    def __swap(self, block_idx: int, key: int, surface: pygame.Surface, dirty_rects: list):
        logging.debug(f'block {block_idx} is swaiped to {self.ENUM2DIR[key]}.')
        block = self.__blocks[block_idx]

        if logging.root.level == logging.DEBUG:
            [logging.debug(f'block {block_idx} [{coord}] is swapped to {self.ENUM2DIR[key]}.') for coord in block.coords]
        ...
        while not all(is_swapped):
            ...
            space_coord = (
                block_coord[0] + self.ENUM2DIR[key][0],
                block_coord[1] + self.ENUM2DIR[key][1]
            )
        ...
        for i in range(len(field)):
            for j in range(len(field[i])):
                if field[i][j] == block_idx:
                    block_coords.append((i, j))
        is_connected = {coord: self.__is_connected(coord, field) for coord in block_coords}
